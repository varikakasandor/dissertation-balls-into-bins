...
def create_comparison(save_name, ns=[10, 20, 50, 100], ms=[10, 20, 50, 100], runs=10):
    ...
    one_choice_vals = {str(n): one_choice_simulate_many_runs(runs=runs, n=n, m=m, print_behaviour=False) for n, m in
                       zip(ns, ms)}
    df = df.append({**{"strategy": "one_choice"}, **one_choice_vals}, ignore_index=True)

    two_choice_vals = {str(n): two_choice_simulate_many_runs(runs=runs, n=n, m=m, print_behaviour=False) for n, m in
                       zip(ns, ms)}
