...
from two_thinning.full_knowledge.RL.DQN.neural_network import FullTwoThinningOneHotNet, FullTwoThinningNet, FullTwoThinningRecurrentNet
...
def get_best_model_path(n=N, m=M, nn_type = NN_TYPE):
    ...
    best_model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "saved_models", f"best_{nn_type_str}{n}_{m}.pth")
...
def load_best_model(n=N, m=M, nn_type=NN_TYPE, device=DEVICE):
    model = FullTwoThinningOneHotNet if nn_type=="one_hot" else (FullTwoThinningRecurrentNet if nn_type=="rnn" else FullTwoThinningNet)
...
def compare(n=N, m=M, train_episodes=TRAIN_EPISODES, memory_capacity=MEMORY_CAPACITY, eps_start=EPS_START,
            ...
            eval_runs_train=EVAL_RUNS_TRAIN, eval_runs_eval=EVAL_RUNS_EVAL, patience=PATIENCE, max_load_increase_reward=MAX_LOAD_INCREASE_REWARD,
            print_progress=PRINT_PROGRESS, print_behaviour=PRINT_BEHAVIOUR, device=DEVICE, nn_model=NN_MODEL, nn_type=NN_TYPE):
