...
class ConstantOffsetStrategy(StrategyBase):
    ...
    def find_best_offset(self, min_offset=-10, max_offset=10, runs_per_offset=100):
        best_avg = - self.m - 1
        best_offset = None
        vals = []
        ...
        plt.xticks(np.arange(min_offset, max_offset+1, step=2))
        plt.legend()
        file_name = f"offset_analysis_{self.n}_{self.m}.png"
