...
\begin{itemize}
    \item 
    $\Phi(v)=\max_{x\sim y \in E} \min(v[x], v[y])$ \NOTE{A}{Why doesn't the argument go below the word "max"?}, i.e. finding the worst edge, where no matter what decision function is used, an already highly loade bin will be chosen.
    \item
...
\begin{itemize} 
    ...
    \item train episodes: the number of training episodes. Note that more episodes are strictly better, since early stopping keeps the best model during training. Hence, this hyperparameter is rather set to a sweetspot where there isn't much expected improvement and the code runs fairly quickly. \NOTE{A}{Mention something about overfitting?}
    
    \item patience: to speed up training, if the best model so far is not improved for this many episodes \NOTE{A}{Hm, double check if the term episode has been defined anywhere in my dissertation...}, training stops.
    ...
    \item learning rate: the learning rate of the ADAM optimiser. \NOTE{A}{Maybe I should also allow choosing other optimisers, such as simple SGD?}
...
\NOTE{A}{Graph structures and Strategies are OOP}
