...
Denoting the load vector at timestep $t$ as $v_t$\NOTE{D}{this should also be defined in Chapter 2}\NOTE{T}{Minor: I think $x_t$ or $x^t$ would be a bit more standard}, we call a \TwoThinning (or \KThinning in general) decision strategy \textit{\textbf{slicing}}, if it accepts a primary bin $i$ if and only if its load $v_t[i]$ is less than or equal to a \textit{\textbf{threshold}} $a$ (that can depend on $v_t$)\NOTE{T}{subtle point, do you also want it to depend on the history of the process? This may impact whether one of the Feldheim strategies is slicing or not...}.
\end{definition}
\NOTE{D}{These are called threshold processes, in here: \url{https://arxiv.org/pdf/2107.03916.pdf\#page=4}}\NOTE{T}{I was also suggest the name threshold (and not only because we used it...)}
