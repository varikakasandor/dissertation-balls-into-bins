...
\NOTE{T}{i would put the caption at the bottom, I think it's more standard.}
\begin{table}[h!]
...
Strategy                                & $m=5$ & $m=10$ & $m=15$ & $m=20$ & $m=60$ & $m=400$ & $m=50$ & $m=200$ & $m=2500$ \\ \hline
...
One of the main conclusions and main surprise of the comparison is that the Local Reward Optimiser Strategy (which rejects only the maximum loads) has similar performance as the Deep Q-Learning Strategy, which was trained to optimise cumulative rewards. This demonstrates that the RL problem underlying \textsc{Two-Thinning} is a tough\NOTE{T}{language, better: challenging} one, in particular, learning long-term consequences of decisions is difficult, because the impact of a single ball is very subtle. In general, making a small number of bad decisions might not even impact the final maximum load. \NOTE{A}{TODO: elaborate some more? This is a crucial point to make.}
...
\NOTE{A}{Explain how the wandb importance tool works.}
...
We can see in the figure that the green region contains the counterexamples to the Greedy strategy, but there is an even larger region where the greedy choice is correct, and characterising the shapes of the regions is difficult. By using a neighbourhood average based potential function for Deep Q-Learning, the agent is guided towards choosing $2$rd bin, and by using a graph-oblivious maximum load potential, the agent is guided towards choosing the $3$rd bin. Therefore, neither of the potential functions is perfect. Since I used the the neighbourhood average based potential function, the resulting algorithm is better at avoiding the pitfalls of Greedy, but in the more dominant red region it tends to make bad decisions. \NOTE{A}{I haven't actually checked this last sentence... Maybe not even needed?}
